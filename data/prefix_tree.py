...
class SimplePrefixTree(Autocompleter):
        ...
        if len(prefix) == len(common_prefix):
            ...
            for subtree in self.subtrees:
                if subtree.value == common_prefix:
                    ...
                    leaf_exists, leaf_index = SimplePrefixTree._find_leaf\
                        (subtree, value, leaf_exists, leaf_index)
            ...
            if not value_duplicate:
                ...
                if prefix == []:
                    ...
                    final_prefix = self._final_leaf(value, weight, prefix)
            ...
            for subtree in self.subtrees:
                if subtree.value == common_prefix:
                    ...
                    self._exact_prefix_found(subtree, weight)
            ...
            if not duplicate_found:
                ...
                prefix_tree._recursion_helper(prefix, common_prefix, weight,
                                              value)
    ...
    def _new_tree(self, prefix: List, weight: float) -> SimplePrefixTree:
    ...
    def _final_leaf(self, value: Any, weight: float, prefix: List) \
            -> SimplePrefixTree:
    ...
    def _start_helper(self, weight: float) -> None:
        self.weight = weight  # starting weight that will be updated
        self.sort_subtrees()

    @staticmethod
    def _find_leaf(subtree: SimplePrefixTree, value: Any,
                   leaf_exists: bool, leaf_index: int) -> Tuple[bool, int]:
        ...
        return leaf_exists, leaf_index

    def _exact_prefix_found(self, subtree: SimplePrefixTree, weight: float
                            ) -> None:
        subtree.weight += weight
        subtree._leaf_total += weight
        self.sort_subtrees()
    ...
    def _recursion_helper(self, prefix: List, common_prefix: List,
                          weight: float, value: List) -> None:
        self._search_val = prefix[0: len(common_prefix) + 1]
...
class CompressedPrefixTree(SimplePrefixTree):
    ...
    def insert(self, value: Any, weight: float, prefix: List) -> None:
        """Insert function that will check whether or not the first value is
        empty or not and act accordingly
        """
...
if __name__ == '__main__':
    ...
    testing_tree = CompressedPrefixTree("sum")
    ...
    # tree = CompressedPrefixTree('sum')
    # tree.insert('hey', 3, ['h', 'e', 'y'])
    # tree.insert('heyhey', 2, ['h', 'e', 'y','h', 'e', 'y'])
    #
    # tree.insert('h', 2, ['h'])
...
# removing taking too long on letter autocomplete -
