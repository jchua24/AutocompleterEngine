...
class SimplePrefixTree(Autocompleter):
    ...
    def insert(self, value: Any, weight: float, prefix: List) -> None:
        ...
        if self.value == []:
            ...
            #update weight of the top SimplePrefixTree
            if self._weight_type == "sum":
                self.weight += weight
            elif self._weight_type == "average":
                #numerator = self.weight + weight
                #denominator = (len(self) + 1)



                #self.weight = (self.weight * len(self) + weight) / (len(self) + 1)
            ...
            for subtree in self.subtrees:
                if subtree.value == common_prefix:
                    ...
                    if subtree._weight_type == "sum":
                        ...
                        subtree.weight = (subtree.weight * len(subtree) + weight) / (
                                    len(subtree) + 1)
                    ...
                    # #subtree.subtrees[0].weight += weight
                    #
                    if subtree.subtrees[0]._weight_type == "sum":
                        subtree.subtrees[0].weight += weight
                    elif subtree.subtrees[0]._weight_type == "average":
                        subtree.subtrees[0].weight = (subtree.subtrees[0].weight * len(subtree.subtrees[0]) + weight) / (
                                    len(subtree.subtrees[0]) + 1)
...
if __name__ == '__main__':
    ...
    # #print(testing_tree)
    #
    #testing_tree.insert("fhat", 2.0, ['f', 'h', 'a', 't'])
    #
    testing_tree.insert("fall", 4.0, ['f', 'a', 'l', 'l'])

    print(testing_tree)

    #search_results = testing_tree.autocomplete([])
    #search_results = testing_tree.autocomplete([], 5)
    search_results = testing_tree.autocomplete([], )
    #search_results = testing_tree.autocomplete(['c', 'h'], 1)
